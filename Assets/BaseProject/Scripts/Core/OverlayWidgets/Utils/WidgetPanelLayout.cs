    ...
    public class WidgetPanelLayout
    {
            ...
            where T : IWidget
        {
            ...
            int GetSize(int size, int defaultValue, int container, int min, int max)
            {
                ...
                if (size == LayoutParams.UseDefault)
                    value = defaultValue;
                else if (size == LayoutParams.UseContainerSize)
                    value = container;
            ...
            for (var i = 0; i < widgets.Count; i++)
            {
                ...
                var width = GetSize(widget.LayoutParams.width, widget.Size.defaultWidth, 
                    (int) paddedRect.width, widget.Size.minWidth, widget.Size.maxWidth);
                
                var height = GetSize(widget.LayoutParams.height, widget.Size.defaultHeight, 
