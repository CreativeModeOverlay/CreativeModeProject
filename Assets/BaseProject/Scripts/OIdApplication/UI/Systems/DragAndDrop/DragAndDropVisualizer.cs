...
namespace CreativeMode
{
    ...
    {
        public GenericPanel itemPrefab;
        ...
        private void Update()
        {
            for (var i = 0; i < instances.Count; i++)
            {
                ...
                CreativeStandaloneInputModule.Instance.GetPointerData(instance.pointerId, out var pointer);
                MoveToPointerPosition(instance.panel, pointer.position);
            }
        }

        private void AnimateAppear(GenericPanel panel)
        {
        ...
        private void MoveToPointerPosition(GenericPanel panel, Vector2 position)
        {
            panel.RectTransform.localPosition = position;
        }

        private void AnimateDisappear(GenericPanel panel, bool isSuccess)
        {
            DOTween.Sequence()
